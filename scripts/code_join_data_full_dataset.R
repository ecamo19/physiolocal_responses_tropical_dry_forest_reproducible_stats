...
## Biomass data -----------------------------------------------------------------
raw_data_biomass <-
    read_excel("./raw_data/5_plant_dry_weights_data.xlsx") %>%
	clean_names()

## Ecophys data -----------------------------------------------------------------
raw_data_ecophys <-
    read_excel("./raw_data/3_physiology_data.xlsx") %>%
    clean_names() %>%
    dplyr::select(- c(family))

## Leaf trait data --------------------------------------------------------------
raw_data_traits <-
    read_excel("./raw_data/2_leaf_trait_data.xlsx") %>%
    clean_names()

## Isotopes data ----------------------------------------------------------------
raw_data_isotopes <-
    read_excel("./raw_data/4_nitrogen_data.xlsx") %>%
    clean_names()
...
data_biomass_cleaned <-
	raw_data_biomass %>%
        ...
        mutate(total_biomass = root_dry_weight_grams + stem_dry_weight_grams +
                               whole_leaf_dry_weight_grams,


            # Mass fractions
            # # % Root Mass Fraction
            rmf = (root_dry_weight_grams / total_biomass) * 100,

            # % Stem Mass Fractions
            smf = (stem_dry_weight_grams / total_biomass) * 100,

            # % Leaf Mass Fraction
            lmf = (whole_leaf_dry_weight_grams / total_biomass) * 100,

            # Root to shoot ratio
            root_shoot_ratio = root_dry_weight_grams/(stem_dry_weight_grams + 
                                                     whole_leaf_dry_weight_grams)) %>%

        # Order Columns
        dplyr::select(id, spcode, treatment, rmf, smf, lmf,
                      whole_leaf_dry_weight_grams, everything()) %>% 
    ...
    data_complete %>%
        ...
        dplyr::mutate(nfixer = factor(ifelse(spcode == "ec" |
               ...
               # Transform Nitrogen percentage to grams
                N_g = (leaf_dry_weight*percentage_n)/100,
                ...
                N_mg = ((leaf_dry_weight*percentage_n)/100)*1000,
